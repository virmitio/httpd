...
CPP=cl.exe
CPP_PROJ=/nologo /W3 /Zi /I "../ssl" /I "../../include" /I "$(APR_INC)" /I "$(APU_INC)" $(CPP_VAR) /I "../generators" /D "WIN32" /D "_WINDOWS" /D "PROXY_DECLARE_EXPORT" /Fo"$(INTDIR)\\" /Fd"$(INTDIR)\mod_proxy_src" /FD /c 
...
RSC=rc.exe
RSC_PROJ=/l 0x409 /fo"$(INTDIR)\mod_proxy.res" /i "../../include" /i "$(APR_INC)"  $(RSC_VAR)  /d BIN_NAME="mod_proxy.so" /d LONG_NAME="proxy_module for Apache"  /i "\tmp\httpd\build\win32" 
BSC32=bscmake.exe
BSC32_FLAGS=/nologo /o"$(OUTDIR)\mod_proxy.bsc" 
BSC32_SBRS= \
	
LINK32=link.exe
LINK32_FLAGS=kernel32.lib ws2_32.lib mswsock.lib $(APR_LIB) $(APU_LIB) $(LINK32_VAR) /libpath:"$(LIB_DIR)" /nologo /subsystem:windows /dll /incremental:no /pdb:"$(OUTDIR)\mod_proxy.pdb" /debug /machine:$(ARCH) /out:"$(OUTDIR)\mod_proxy.so" /implib:"$(OUTDIR)\mod_proxy.lib" /base:@..\..\os\win32\BaseAddr.ref,mod_proxy.so 
LINK32_OBJS= \
...
TargetPath=$(OUTDIR)\mod_proxy.so
SOURCE="$(InputPath)"
...
$(DS_POSTBUILD_DEP) : "$(OUTDIR)\mod_proxy.so"
   if exist $(OUTDIR)\mod_proxy.so.manifest mt.exe -manifest $(OUTDIR)\mod_proxy.so.manifest -outputresource:$(OUTDIR)\mod_proxy.so;2
