...
!IF "$(RECURSE)" == "1" 
!IF  "$(CFG)" == "mod_allowmethods - Win32 Release"
ALL : "libhttpd - Win32 Release" "$(OUTDIR)\mod_allowmethods.so"
CLEAN :"libhttpd - Win32 ReleaseCLEAN" 
!ELSE 
ALL : "libhttpd - Win32 Debug" "$(OUTDIR)\mod_allowmethods.so"
...
CPP=cl.exe
CPP_PROJ=/nologo /W3 /Zi /I "../../include" /I "$(APR_INC)" /I "$(APU_INC)" $(CPP_VAR) /D "WIN32" /D "_WINDOWS" /Fo"$(INTDIR)\\" /Fd"$(INTDIR)\mod_allowmethods_src" /FD /c 
...
RSC=rc.exe
RSC_PROJ=/l 0x409 /fo"$(INTDIR)\mod_allowmethods.res" /i "../../include" /i "$(APR_INC)"  $(RSC_VAR)  /d BIN_NAME="mod_allowmethods.so" /d LONG_NAME="allowmethods_module for Apache"  /i "\tmp\httpd\build\win32" 
BSC32=bscmake.exe
BSC32_FLAGS=/nologo /o"$(OUTDIR)\mod_allowmethods.bsc" 
BSC32_SBRS= \
	
LINK32=link.exe
LINK32_FLAGS=kernel32.lib $(APR_LIB) $(APU_LIB) $(LINK32_VAR) /libpath:"$(LIB_DIR)" /nologo /subsystem:windows /dll /incremental:no /pdb:"$(OUTDIR)\mod_allowmethods.pdb" /debug /machine:$(ARCH) /out:"$(OUTDIR)\mod_allowmethods.so" /implib:"$(OUTDIR)\mod_allowmethods.lib" /base:@..\..\os\win32\BaseAddr.ref,mod_allowmethods.so 
LINK32_OBJS= \
...
TargetPath=$(OUTDIR)\mod_allowmethods.so
SOURCE="$(InputPath)"
...
$(DS_POSTBUILD_DEP) : "$(OUTDIR)\mod_allowmethods.so"
   if exist $(OUTDIR)\mod_allowmethods.so.manifest mt.exe -manifest $(OUTDIR)\mod_allowmethods.so.manifest -outputresource:$(OUTDIR)\mod_allowmethods.so;2
