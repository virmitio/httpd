...
CPP=cl.exe
CPP_PROJ=/nologo /W3 /Zi /I "../../../include" /I "$(APR_INC)" /I "$(APU_INC)" $(CPP_VAR) /D "WIN32" /D "_WINDOWS" /Fo"$(INTDIR)\\" /Fd"$(INTDIR)\mod_isapi_src" /FD /c 
...
RSC=rc.exe
RSC_PROJ=/l 0x409 /fo"$(INTDIR)\mod_isapi.res" /i "../../../include" /i "$(APR_INC)"  $(RSC_VAR)  /d BIN_NAME="mod_isapi.so" /d LONG_NAME="isapi_module for Apache"  /i "\tmp\httpd\build\win32" 
BSC32=bscmake.exe
BSC32_FLAGS=/nologo /o"$(OUTDIR)\mod_isapi.bsc" 
BSC32_SBRS= \
	
LINK32=link.exe
LINK32_FLAGS=kernel32.lib ws2_32.lib mswsock.lib $(APR_LIB) $(APU_LIB) $(LINK32_VAR) /libpath:"$(LIB_DIR)" /nologo /subsystem:windows /dll /incremental:no /pdb:"$(OUTDIR)\mod_isapi.pdb" /debug /machine:$(ARCH) /out:"$(OUTDIR)\mod_isapi.so" /implib:"$(OUTDIR)\mod_isapi.lib" /base:@..\..\..\os\win32\BaseAddr.ref,mod_isapi.so 
LINK32_OBJS= \
...
TargetPath=$(OUTDIR)\mod_isapi.so
SOURCE="$(InputPath)"
...
$(DS_POSTBUILD_DEP) : "$(OUTDIR)\mod_isapi.so"
   if exist $(OUTDIR)\mod_isapi.so.manifest mt.exe -manifest $(OUTDIR)\mod_isapi.so.manifest -outputresource:$(OUTDIR)\mod_isapi.so;2
