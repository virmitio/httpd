...
CPP=cl.exe
CPP_PROJ=/nologo /W3 /Zi /I "$(APR_INC)" /I "$(APU_INC)" $(CPP_VAR) /I "../../include" /D "WIN32" /D "_WINDOWS" /Fo"$(INTDIR)\\" /Fd"$(INTDIR)\mod_charset_lite_src" /FD /c 
...
RSC=rc.exe
RSC_PROJ=/l 0x409 /fo"$(INTDIR)\mod_charset_lite.res" /i "../../include" /i "$(APR_INC)"  $(RSC_VAR)  /d BIN_NAME="mod_charset_lite.so" /d LONG_NAME="charset_lite_module for Apache"  /i "\tmp\httpd\build\win32" 
BSC32=bscmake.exe
BSC32_FLAGS=/nologo /o"$(OUTDIR)\mod_charset_lite.bsc" 
BSC32_SBRS= \
	
LINK32=link.exe
LINK32_FLAGS=kernel32.lib $(APR_LIB) $(APU_LIB) $(LINK32_VAR) /libpath:"$(LIB_DIR)" /nologo /subsystem:windows /dll /incremental:no /pdb:"$(OUTDIR)\mod_charset_lite.pdb" /debug /machine:$(ARCH) /out:"$(OUTDIR)\mod_charset_lite.so" /implib:"$(OUTDIR)\mod_charset_lite.lib" /base:@..\..\os\win32\BaseAddr.ref,mod_charset_lite.so 
LINK32_OBJS= \
...
TargetPath=$(OUTDIR)\mod_charset_lite.so
SOURCE="$(InputPath)"
...
$(DS_POSTBUILD_DEP) : "$(OUTDIR)\mod_charset_lite.so"
   if exist $(OUTDIR)\mod_charset_lite.so.manifest mt.exe -manifest $(OUTDIR)\mod_charset_lite.so.manifest -outputresource:$(OUTDIR)\mod_charset_lite.so;2
