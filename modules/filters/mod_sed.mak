...
CPP=cl.exe
CPP_PROJ=/nologo /W3 /Zi /I "../../include" /I "$(APR_INC)" /I "$(APU_INC)" $(CPP_VAR) /D "WIN32" /D "_WINDOWS" /Fo"$(INTDIR)\\" /Fd"$(INTDIR)\mod_sed_src" /FD /c 
...
RSC=rc.exe
RSC_PROJ=/l 0x409 /fo"$(INTDIR)\mod_sed.res" /i "../../include" /i "$(APR_INC)"  $(RSC_VAR)  /d BIN_NAME="mod_sed.so" /d LONG_NAME="substitute_module for Apache"  /i "\tmp\httpd\build\win32" 
BSC32=bscmake.exe
BSC32_FLAGS=/nologo /o"$(OUTDIR)\mod_sed.bsc" 
BSC32_SBRS= \
	
LINK32=link.exe
LINK32_FLAGS=kernel32.lib $(APR_LIB) $(APU_LIB) $(LINK32_VAR) /libpath:"$(LIB_DIR)" /nologo /subsystem:windows /dll /incremental:no /pdb:"$(OUTDIR)\mod_sed.pdb" /debug /machine:$(ARCH) /out:"$(OUTDIR)\mod_sed.so" /implib:"$(OUTDIR)\mod_sed.lib" /base:@..\..\os\win32\BaseAddr.ref,mod_sed.so 
LINK32_OBJS= \
	"$(INTDIR)\mod_sed.obj" \
	"$(INTDIR)\regexp.obj" \
	"$(INTDIR)\sed0.obj" \
...
$(DS_POSTBUILD_DEP) : "$(OUTDIR)\mod_sed.so"
   if exist $(OUTDIR)\mod_sed.so.manifest mt.exe -manifest $(OUTDIR)\mod_sed.so.manifest -outputresource:$(OUTDIR)\mod_sed.so;2
