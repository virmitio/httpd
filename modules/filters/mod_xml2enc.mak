...
CPP=cl.exe
CPP_PROJ=/nologo /W3 /Zi /I "../../include" /I "$(APR_INC)" /I "$(APU_INC)" $(CPP_VAR) /I "../../srclib/libxml2/include" /D "WIN32" /D "_WINDOWS" /Fo"$(INTDIR)\\" /Fd"$(INTDIR)\mod_xml2enc_src" /FD /c 
...
LINK32=link.exe
LINK32_FLAGS=kernel32.lib libxml2.lib $(APR_LIB) $(APU_LIB) $(LINK32_VAR) /libpath:"$(LIB_DIR)" /nologo /subsystem:windows /dll /incremental:no /pdb:"$(OUTDIR)\mod_xml2enc.pdb" /debug /machine:$(ARCH) /out:"$(OUTDIR)\mod_xml2enc.so" /implib:"$(OUTDIR)\mod_xml2enc.lib" /libpath:"../../srclib/libxml2/win32/bin.msvc" /base:@..\..\os\win32\BaseAddr.ref,mod_xml2enc.so 
LINK32_OBJS= \
...
TargetPath=$(OUTDIR)\mod_xml2enc.so
SOURCE="$(InputPath)"
...
$(DS_POSTBUILD_DEP) : "$(OUTDIR)\mod_xml2enc.so"
   if exist $(OUTDIR)\mod_xml2enc.so.manifest mt.exe -manifest $(OUTDIR)\mod_xml2enc.so.manifest -outputresource:$(OUTDIR)\mod_xml2enc.so;2
